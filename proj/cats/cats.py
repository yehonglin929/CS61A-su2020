...
def accuracy(typed, reference):
    ...
    for i in range(min(len(typed_words), len(reference_words))):
        typed_words[i] = typed_words[i].replace('\t', '')
        reference_words[i] = reference_words[i].replace('\t', '')
...
def time_per_word(times_per_player, words):
    ...
    "*** YOUR CODE HERE ***"
    time_list = []
    for player_idx in range(len(times_per_player)):
        time_list.append([])
        for word_idx in range(1, len(times_per_player[player_idx])):
            time_list[-1].append(times_per_player[player_idx][word_idx] - times_per_player[player_idx][word_idx-1])
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fastest = [[] for i in range(len(players))]
    for word_idx in words:
        min_time = time(game, 0, word_idx)
        min_player = 0
        for player_idx in players[1:]:
            t = time(game, player_idx, word_idx)
            if t < min_time:
                min_time = t
                min_player = player_idx
        fastest[min_player].append(word_at(game, word_idx))
    return fastest
