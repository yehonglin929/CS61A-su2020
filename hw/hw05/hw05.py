...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, psw):
        ...
        if len(wrong_passwords) == 3:
            return "Too many incorrect attempts. Attempts: ['{}', '{}', '{}']".format(*wrong_passwords)
        if password != psw:
            wrong_passwords.append(psw)
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    """
    "*** YOUR CODE HERE ***"
    new_w = withdraw(0, old_pass)
    if type(new_w) is str:
        return new_w
    def joint_withdraw(amount, psw):
        if psw == new_pass:
...
def remainders_generator(m):
    ...
    def gene(offset, base):
        ...
        while True:
            begin += base
            yield begin
    return iter([gene(i, m) for i in range(m)])
