...
def has_path(t, phrase):
    ...
    for br in branches(t):
        find = find or has_path(br, phrase[1:])
    return find
...
def quadratic(x, a, b, c):
    ...
    """
    "*** YOUR CODE HERE ***"
    new_lower_bound = quad(lower_bound(x), a, b, c)
    new_upper_bound = quad(upper_bound(x), a, b, c)
    if a != 0 and -b/(2*a) < upper_bound(x) and -b/(2*a) > lower_bound(x):
        bound = quad(-b/(2*a), a, b, c)
        return interval(min(new_lower_bound, new_upper_bound, bound), max(new_lower_bound, new_upper_bound, bound))
    else:
        return interval(min(new_lower_bound, new_upper_bound), max(new_lower_bound, new_upper_bound))
