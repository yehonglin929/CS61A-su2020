...
def subseqs(s):
    ...
    if len(s) == 0:
        ...
        sseqs = subseqs(s[1:])
        return sseqs + insert_into_all(s[0], sseqs)
...
def make_glookup(class_assignments):
    ...
    assign_score = 0
    total_score = 0
    def inner_glookup(assign, score):
        nonlocal assign_score, total_score
        assign_score += score
        total_score += class_assignments[assign]
        return assign_score / total_score
    return inner_glookup
...
def make_advanced_counter_maker():
    ...
    def make_counter():
        name_count = 0
        def name_counter(tp):
            nonlocal global_count, name_count
            "*** YOUR CODE HERE ***"
            if tp == 'count':
                name_count += 1
                return name_count
            elif tp == 'global-count':
                global_count += 1
                return global_count
            elif tp == 'reset':
                name_count = 0
